...
cp theme uses [Webpack](https://webpack.js.org) to compile and
bundle SASS and JS.
...
Go to the root of cp theme and run the following commands:
`nvm use`
...
`drush en cp -y`
`drush config-set system.theme default cp -y`
